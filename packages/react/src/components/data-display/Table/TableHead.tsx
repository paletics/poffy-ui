'use client';

import { Slot } from '@radix-ui/react-slot';
import { cx } from '@/styled-system/css';
import { ElementType, forwardRef } from 'react';
import { useTableContext } from './TableContext';
import { TableHeadProps } from './Table.types';
import { getTableFallbackChildren, isTableAsChildHost } from './Table.utils';

/**
 * Renders the native `thead` styled by the owning Table.
 *
 * `asChild` accepts only `thead`; unsupported delegated content retains only
 * row children before falling back to the default element.
 */
export const TableHead = forwardRef<HTMLTableSectionElement, TableHeadProps>((props, ref) => {
  const { asChild, children, className, ...rest } = props;
  const { classes } = useTableContext();
  const canUseAsChild = Boolean(asChild && isTableAsChildHost(children, ['thead']));
  const renderedChildren = canUseAsChild
    ? children
    : asChild
      ? getTableFallbackChildren(children, ['tr'])
      : children;
  const Component = (canUseAsChild ? Slot : 'thead') as ElementType;

  return (
    <Component ref={ref} className={cx(classes.head, className)} {...rest}>
      {renderedChildren}
    </Component>
  );
});

TableHead.displayName = 'Table.Head';
